<template>
    <div class="card">
        <Breadcrumb :items="[{ label: 'Product' }, { label: 'List Products' },]"/>
        ...
        >
            ...
            <Column field="stripe_price_id" header="Stripe price id">
                <template #body="{ data }">
                    <span>{{ data.stripe_price_id || '-' }}</span>
                </template>
                <template #filter="{ filterModel }">
                    <InputText v-model="filterModel.value" type="text" placeholder="Search by stripe price id"/>
                </template>
            </Column>

            <Column field="token_qty" header="Token qty">
                <template #body="{ data }">
